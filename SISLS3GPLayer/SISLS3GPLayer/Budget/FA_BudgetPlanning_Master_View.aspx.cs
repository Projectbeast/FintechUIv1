...
public partial class FA_BudgetPlanning_Master_View : ApplyThemeForProject
{
    ...
    public void FunPriBindGrid()
    {
        try
        {
            ...
            //this.grvBudgetPlanningMasterView.DataSource = Dset.Tables[0];
            //this.grvBudgetPlanningMasterView.DataBind();
            ...
            grvBudgetPlanningMasterView.BindGridView_FA("FA_GET_BUDGET_MASTER_LIST", ProcValue, out  intTotalRecords, ObjPaging_FA, out bIsNewRow);
            grvBudgetPlanningMasterView.DataBind();

            if (bIsNewRow)
            {
                grvBudgetPlanningMasterView.Rows[0].Visible = false;
            }
    ...
    {
        if (grvBudgetPlanningMasterView.HeaderRow != null)
        {
            ((TextBox)grvBudgetPlanningMasterView.HeaderRow.FindControl("txtHeaderSearch")).Text = strSearchVal1;
            ((TextBox)grvBudgetPlanningMasterView.HeaderRow.FindControl("txtHeaderGLSearch")).Text = strSearchGLVal;
            ((TextBox)grvBudgetPlanningMasterView.HeaderRow.FindControl("txtHeaderActivitySearch")).Text = strSearchActivityVal;
    ...
    private void FunPriGetSearchValue_FA()
    {
        try
        {
            if (grvBudgetPlanningMasterView.HeaderRow != null)
            {
                strSearchVal1  = ((TextBox)grvBudgetPlanningMasterView.HeaderRow.FindControl("txtHeaderSearch")).Text.Trim();
                strSearchGLVal = ((TextBox)grvBudgetPlanningMasterView.HeaderRow.FindControl("txtHeaderGLSearch")).Text.Trim();
                strSearchActivityVal = ((TextBox)grvBudgetPlanningMasterView.HeaderRow.FindControl("txtHeaderActivitySearch")).Text.Trim();
    ...
    private void FunPriClearSearchValue_FA()
    {
        try
        {
            if (grvBudgetPlanningMasterView.HeaderRow != null)
            {
                ((TextBox)grvBudgetPlanningMasterView.HeaderRow.FindControl("txtHeaderSearch")).Text = "";
    ...
    protected void FunProSortingColumn(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (strDirection == "ASC")
            {
                ((ImageButton)grvBudgetPlanningMasterView.HeaderRow.FindControl(imgbtnSearch)).CssClass = "styleImageSortingAsc";
            }
            else
            {
                ((ImageButton)grvBudgetPlanningMasterView.HeaderRow.FindControl(imgbtnSearch)).CssClass = "styleImageSortingDesc";
            }
        }
        catch (Exception ex)
        {
    ...
    protected void FunProHeaderSearch(object sender, EventArgs e)
   {
        ...
        try
        {
            ...
            if (strSearchVal1 != "")
            {
                strSearchVal += " and Upper(fin_year) like '%" + strSearchVal1.ToUpper() + "%'";
            }

            if (strSearchGLVal != "")
            {
                strSearchVal += " and Upper(GL_ACCOUNT) like '%" + strSearchGLVal.ToUpper() + "%'";
            }

            if (strSearchActivityVal != "")
            {
                strSearchVal += " and Upper(Activity) like '%" + strSearchActivityVal.ToUpper() + "%'";
            }
            ...
            if (txtboxSearch.Text != "")
                ((TextBox)grvBudgetPlanningMasterView.HeaderRow.FindControl(txtboxSearch.ID)).Focus();
        }
        catch (Exception ex)
        {
            FAClsPubCommErrorLog.CustomErrorRoutine(ex, strPageName);
