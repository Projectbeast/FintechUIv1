...
public partial class System_Admin_S3G_SysAdminAccountSetupMaster_View : ApplyThemeForProject
{
    ...
    protected void FunProHeaderSearch(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (strSearchVal2 != "")
            {
                strSearchVal += " and (BM.Branch_CODE +' - '+ BM.BRANCH_NAME) like '" + strSearchVal2 + "%'";

            }
            if (strSearchVal3 != "")
            {
                strSearchVal += " and Account_Flag like '" + strSearchVal3 + "%'";
            }
    ...
    private string FunPriGetSortDirectionStr(string strColumn)
    {
        ...
        if (strColumn == "BRANCH")
            strColumn = "BM.Branch_CODE +' - '+ BM.BRANCH_NAME";
