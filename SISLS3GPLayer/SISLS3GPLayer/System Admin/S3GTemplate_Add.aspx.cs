...
public partial class System_Admin_S3GTemplate_Add : ApplyThemeForProject
{
    ...
    public static System_Admin_S3GTemplate_Add obj_Page;
    ...
    protected void ddlTemplateType_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddlTemplateType.SelectedValue == "31") //For SMS Offer Alert
                lblExclusion.Text = "SMS To";
            else
                lblExclusion.Text = "Exclusion";

            ddlTemplateType.Focus();//Added by Suseela
        }
        catch (Exception ex)
        {
    ...
    protected void ddlModeofMail_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            FunPriSetModeofMail();
            ddlModeofMail.Focus();//Added by Suseela
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            String value = HttpUtility.HtmlDecode(FTBTemplate.Text);
            value = value.Trim();
            if (!(value.Length > 0))
            {
                Utility.FunShowAlertMsg(this.Page, "Template Content cannot be left empty");
                return;
            }
            if (ddlLocation.SelectedText != String.Empty && ddlLocation.SelectedValue == "0")
            {
                Utility.FunShowAlertMsg(this.Page, "Enter the valid Branch");
                ddlLocation.SelectedText = "";
                return;
            }
            FunPriSave();
        }
    ...
    private void FunPriLoadCustomerDtls()
    {
        try
        {
            UserControls_LOBMasterView ucCustomerLov = GrvExclusion.FooterRow.FindControl("ucCustomerLov") as UserControls_LOBMasterView;
            DropDownList ddlFooterCategory = (DropDownList)GrvExclusion.FooterRow.FindControl("ddlFooterCategory");
            HiddenField hFooCategVal = (HiddenField)GrvExclusion.FooterRow.FindControl("hFooCategVal");
            if (ddlFooterCategory.SelectedIndex == 0)
            {
                Utility.FunShowAlertMsg(this, "Category cannot be left empty");
                return;
            }
            else
            ...
            if (ddlFooterCategory.SelectedValue != hFooCategVal.Value)
            {
                hFooCategVal.Value = ddlFooterCategory.SelectedValue;
    ...
    protected void FunProLoadLOB()
    {
        try
        {
            ...
            Procparam.Add("@User_ID", ObjUserInfo.ProUserIdRW.ToString());
            Procparam.Add("@Program_ID", "215");
            Procparam.Add("@TYPE", "LANGUAGE_TYPE");
            ddlLanguage.BindDataTable("S3G_GET_ORG_LOOKUP", Procparam, new string[] { "VALUE", "NAME" });
            Procparam = null;
        }
        catch (Exception ex)
        {
        ...
        try
        {
            ...
            switch (intModeID)
            {
                ...
                case -1:// Query Mode
                    ...
                    ddlLOB.ClearDropDownList();
                    ddlModeofMail.ClearDropDownList();
                    ddlLanguage.ClearDropDownList();
                    chkIsActive.Enabled = ddlLocation.Enabled = txtDescription.Enabled = false;
    ...
    private void FunPriClear()
    {
        try
        {
            ddlLOB.SelectedIndex = ddlTemplateType.SelectedIndex = ddlModeofMail.SelectedIndex = ddlLanguage.SelectedIndex = -1;
            txtDescription.Text = txtDocumentPath.Text = string.Empty;
    ...
    private void FunPriSave()
    {
        try
        {
            ...
            ObjTemplateDtlsRow.Template_Content = Convert.ToString(FTBTemplate.Text).Replace("&nbsp;", " ").Replace("&NBSP;", " ");
            ...
            {
                Utility.FunShowAlertMsg(this.Page, "Document Number Control Not Defined");
                return;
            }
            else if (intErrorCode == 4)
            {
                Utility.FunShowAlertMsg(this.Page, "Location Level Record already exists");
                return;
            }
            else if (intErrorCode == 5)
            {
                Utility.FunShowAlertMsg(this.Page, "Selected Combination Already Exists");
                return;
            }
            else if (intErrorCode == 50)
            {
                Utility.FunShowAlertMsg(this.Page, "Error In Saving the Template Details");
                return;
            }
            else
            {
    ...
    //    #endregion
    //    ddlLanguage.Focus();//Added by Suseela
    //}
