...
namespace FA_DALayer.FinancialAccounting
{
    public class ClsPubManualJournal
    {
        ...
        Database db;
        public ClsPubManualJournal(string strConnectionName)
        {
        ...
        public int FunPubInsertManualJournal(FASerializationMode SerMode, byte[] bytesobjFATran_Header_DTB, string strConnectionName, out int intMJV_ID, out string strDocNo, out string strErrorMsg)
        {
            ...
            try
            {
                ...
                {
                    if (objFATran_Header_Row.Tran_Type == "FAMJ")
                    {
                        if (objFATran_Header_Row.Option == 1)
                            command = db.GetStoredProcCommand(SPNames.InsertMJV);
                        else if (objFATran_Header_Row.Option == 2)
                            command = db.GetStoredProcCommand(SPNames.UpdateMJV);
                    }
                    else
                    {
                        command = db.GetStoredProcCommand("FA_INS_PRIORJV");
                    }
                    ...
                    if (!objFATran_Header_Row.IsXML_Tran_AUTHNull())
                    {
                        ...
                        if (enumDBType == FADALDBType.ORACLE)
                        {
                            OracleParameter param = new OracleParameter("@XML_Tran_AUTH", OracleType.Clob,
                                        objFATran_Header_Row.XML_Tran_MG.Length, ParameterDirection.Input, true,
                    ...
                    if (enumDBType == FADALDBType.ORACLE)
                    {
                        OracleParameter param = new OracleParameter("@XML_Tran_Funder", OracleType.Clob,
                    ...
                    {
                        db.AddInParameter(command, "@XML_Tran_Funder", DbType.String, objFATran_Header_Row.XML_Tran_OTH);
                    }
                    ...
                    db.AddInParameter(command, "@Remarks", DbType.String, objFATran_Header_Row.Remarks);
