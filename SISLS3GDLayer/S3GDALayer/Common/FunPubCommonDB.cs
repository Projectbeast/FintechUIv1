    ...
    public static partial class ClsPubCommonDB
    {
        ...
        public static void FunPubGetORACLE_SP(this DbCommand command)
        {
            try
            {
                ...
                StrProcedureName = S3GBusEntity.SPNames_SQL_ORA.ResourceManager.GetString(command.CommandText.Trim().Replace("[", "").Replace("]", "").Trim());
                if (!string.IsNullOrEmpty(StrProcedureName))
                    command.CommandText = StrProcedureName.Trim();
        ...
        public static void FunReplaceOracleParameter(ref DbCommand dbCmd, S3GDALDBType enumDBType)
        {
            try
            {
                for (int i = 0; i <= dbCmd.Parameters.Count - 1; i++)
                {
                    ...
                    }//Modified By : Anbuvel.T, Date : 08-SEP-2018, Description : To Convert Value as Upper Case except of Freetext
                    else if (dbCmd.Parameters[i].DbType == DbType.String)
                    {
                        if (dbCmd.Parameters[i].ParameterName.ToUpper().Contains("REMARKS") 
                            || dbCmd.Parameters[i].ParameterName.ToUpper().Contains("NARRATION") 
                            || dbCmd.Parameters[i].ParameterName.ToUpper().Contains("NOTES") 
                            || dbCmd.Parameters[i].ParameterName.ToUpper().Contains("PASSWORD") 
                            || dbCmd.Parameters[i].ParameterName.ToUpper().Contains("DATE")
                            || dbCmd.Parameters[i].ParameterName.ToUpper().Contains("TEMPLATE_CONTENT")
                            || dbCmd.Parameters[i].ParameterName.ToUpper().Contains("MAIL_BODY"))
                        {
            ...
            try
            {
                ...
                if (strSendError == "1")
                {
                    ...
                    try
                    {
                        ...
                        if (enumDBType == S3GDALDBType.ORACLE)//Added By Thalai For to solve Error Log insert
                        {
                            OracleParameter param;
                            param = new OracleParameter("@ERROR_MESSAGE",
                                   OracleType.Clob, strErrorMsg.Length,
                                   ParameterDirection.Input, true, 0, 0, String.Empty,
                                   DataRowVersion.Default, strErrorMsg);
                        ...
                        if (enumDBType == S3GDALDBType.ORACLE)//Added By Sathish For to solve Error Log insert
                        {
                            OracleParameter param;
                        ...
                        if (enumDBType == S3GDALDBType.ORACLE)//Added By Sathish For to solve Error Log insert
                        {
                            ...
                            param2 = new OracleParameter("@PARAM_TYPE",
                                   OracleType.Clob, strPramType.Length,
                                   ParameterDirection.Input, true, 0, 0, String.Empty,
                                   DataRowVersion.Default, strPramType);
                            dbErrorLogCommand.Parameters.Add(param2);
                        }
                        else
                        {
