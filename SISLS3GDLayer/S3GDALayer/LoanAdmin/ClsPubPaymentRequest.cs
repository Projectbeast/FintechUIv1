...
namespace S3GDALayer.LoanAdmin
{
    namespace LoanAdminAccMgtServices
    {
        public class ClsPubPaymentRequest
        {
            ...
            public int FunPubCreateOrModifyPaymentRequest(SerializationMode SerMode, byte[] bytesObjPaymentRequest_DataTable, out string paynum, out int request_No,out string chqNumber)
            {
                request_No = 0;
                paynum = "";
                chqNumber = string.Empty;
                try
                {
                    ...
                    foreach (S3GBusEntity.LoanAdmin.LoanAdminAccMgtServices.S3G_LOANAD_PaymentRequestRow ObjPaymentRequestRow in ObjPaymentRequest_DataTable.Rows)
                    {
                        ...
                        if (!(string.IsNullOrEmpty(ObjPaymentRequestRow.Payment_Request_No)))
                       ...
                       // db.AddInParameter(command, "@Request_No", DbType.Int32, ObjPaymentRequestRow.Request_No);
                        ...
                        if (!(string.IsNullOrEmpty(ObjPaymentRequestRow.Favouring_name)))
                            db.AddInParameter(command, "@Favouring_name", DbType.String, ObjPaymentRequestRow.Favouring_name); //Favouring Name
                        ...
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Debts_Outstanding) != -1)
                            db.AddInParameter(command, "@Debts_Outstanding", DbType.Decimal, ObjPaymentRequestRow.Debts_Outstanding);
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Debts_Approved) != -1)
                            db.AddInParameter(command, "@Debts_Approved", DbType.Decimal, ObjPaymentRequestRow.Debts_Approved);
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Debts_Un_Approved) != -1)
                            db.AddInParameter(command, "@Debts_Un_Approved", DbType.Decimal, ObjPaymentRequestRow.Debts_Un_Approved);
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Over_Due_Invoice) != -1)
                            db.AddInParameter(command, "@Over_Due_Invoice", DbType.Decimal, ObjPaymentRequestRow.Over_Due_Invoice);
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Default_Invoice) != -1)
                            db.AddInParameter(command, "@Default_Invoice", DbType.Decimal, ObjPaymentRequestRow.Default_Invoice);
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Drawing_Power) != -1)
                            db.AddInParameter(command, "@Drawing_Power", DbType.Decimal, ObjPaymentRequestRow.Drawing_Power);
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Funds_In_Use) != -1)
                            db.AddInParameter(command, "@Funds_In_Use", DbType.Decimal, ObjPaymentRequestRow.Funds_In_Use);
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Available_Funds) != -1)
                            db.AddInParameter(command, "@Available_Funds", DbType.Decimal, ObjPaymentRequestRow.Available_Funds);
                        if (Convert.ToDecimal(ObjPaymentRequestRow.Funds_In_After_Disbursement) != -1)
                            db.AddInParameter(command, "@Funds_In_Use_DB", DbType.Decimal, ObjPaymentRequestRow.Funds_In_After_Disbursement);
                        if (enumDBType == S3GDALDBType.ORACLE)
                        {
                            OracleParameter param;
                            if (ObjPaymentRequestRow.XML_PaymentDetails != null)
                            {
                                param = new OracleParameter("@XML_PaymentDetails", OracleType.Clob,
                                    ObjPaymentRequestRow.XML_PaymentDetails.Length, ParameterDirection.Input, true,
                                    0, 0, String.Empty, DataRowVersion.Default, ObjPaymentRequestRow.XML_PaymentDetails);
                                command.Parameters.Add(param);
                            }

                            if (ObjPaymentRequestRow.XML_PaymentAdjustment != null)
                            {
                                param = new OracleParameter("@XML_PaymentAdjustment", OracleType.Clob,
                                    ObjPaymentRequestRow.XML_PaymentAdjustment.Length, ParameterDirection.Input, true,
                                    0, 0, String.Empty, DataRowVersion.Default, ObjPaymentRequestRow.XML_PaymentAdjustment);
                                command.Parameters.Add(param);
                            }
                        ...
                        if (ObjPaymentRequestRow.DISB_LOCATION_ID != -1)
                            db.AddInParameter(command, "@DISB_LOCATION_ID", DbType.Int32, ObjPaymentRequestRow.DISB_LOCATION_ID);
                        if (ObjPaymentRequestRow.ACCOUNT_ID != -1)
                            db.AddInParameter(command, "@ACCOUNT_ID", DbType.Int32, ObjPaymentRequestRow.ACCOUNT_ID);
                        ...
                        if(ObjPaymentRequestRow.Requestno !=-1)
                            db.AddInParameter(command, "@Requestno", DbType.Int32, ObjPaymentRequestRow.Requestno);
                        // Created By : Anbuvel.T Date : 23-10-2018, Description : Payment Releated Changes Done against for Cheque Return Charges.
                        if (ObjPaymentRequestRow.Is_Hold_Cheque != -1)
                            db.AddInParameter(command, "@Is_Hold_Cheque", DbType.Int16, ObjPaymentRequestRow.Is_Hold_Cheque);
                        if (ObjPaymentRequestRow.Is_Replace_Cheque != -1)
                            db.AddInParameter(command, "@Is_Replace_Cheque", DbType.Int16, ObjPaymentRequestRow.Is_Replace_Cheque);
                        if (ObjPaymentRequestRow.Batch_No != -1)
                            db.AddInParameter(command, "@Batch_No", DbType.Int32, ObjPaymentRequestRow.Batch_No);
                        //if (ObjPaymentRequestRow.Is_Cheque_Print != -1)
                        //    db.AddInParameter(command, "@Is_Cheque_Print", DbType.Int16, ObjPaymentRequestRow.Is_Cheque_Print);
                        db.AddOutParameter(command, "@ErrorCode", DbType.Int32, sizeof(Int32));
                        db.AddOutParameter(command, "@PayNum", DbType.String, 150);
                        db.AddOutParameter(command, "@chqNumber", DbType.String, 50);
                            ...
                            try
                            {
                               // db.ExecuteNonQuery(command, trans);
                                ...
                                if (intRowsAffected == 0)
                                {
                                    ...
                                    chqNumber = (string)command.Parameters["@chqNumber"].Value;
