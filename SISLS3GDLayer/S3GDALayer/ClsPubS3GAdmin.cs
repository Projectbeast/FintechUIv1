...
namespace S3GDALayer
{
    namespace S3GAdminServices
    {
        public class ClsPubS3GAdmin
        {
            ...
            public DataTable FunPubFillDropdownUpload(string strProcName, string strConnectionName, Dictionary<string, string> dctProcParams, string strXMLParameterKey, string strXMLParameterValue) // 2 b edited
            {
            ...
            public DataTable FunPubFillGridPaging(string strProcName, Dictionary<string, string> dctProcParams, out int intTotalRecords, PagingValues ObjPaging, out string IsDynamicSearchFlag)
            {
                intTotalRecords = 0;
                ...
                try
                {
                    ...
                    db.AddInParameter(command, "@OrderBy", DbType.String, ObjPaging.ProOrderBy);
                    db.AddOutParameter(command, "@TotalRecords", DbType.Int32, sizeof(Int32));
                    db.AddOutParameter(command, "@IsSearchFlag", DbType.String, 20);
                    //if (ObjPaging.ProProgram_ID > 0)
                    //db.AddInParameter(command, "@Program_Id", DbType.String, ObjPaging.ProProgram_ID);
                    db.FunPubLoadDataSet(command, ds, "GridTable");
                    //db.LoadDataSet(command, ds, "GridTable")
                    String ColumnName = "Total_Records";
                    DataColumnCollection columns = ds.Tables[0].Columns;
                    String Flag1 = "IsSearchFlag";
                    String Flag2 = "IsPageFlag";
                    DataColumnCollection Flag = ds.Tables[0].Columns;
                    if (ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                    {
                        IsDynamicSearchFlag = "";
                        if (columns.Contains(ColumnName))
                        {
                            intTotalRecords = Convert.ToInt32(ds.Tables[0].Rows[0][ColumnName].ToString());
                            ds.Tables[0].Columns.Remove(ColumnName);
                            ds.Tables[0].AcceptChanges();
                        }
                        else if (command.Parameters["@TotalRecords"].Value != null)
                        {
                            if (((int)command.Parameters["@TotalRecords"].Value != 0) && ((int)command.Parameters["@TotalRecords"].Value > 0))
                                intTotalRecords = (int)command.Parameters["@TotalRecords"].Value;
                        }
                    }
                    else if (columns.Contains(ColumnName) || Flag.Contains(Flag1) || Flag.Contains(Flag2))
                    {
                        if (Flag.Contains(Flag1))
                            IsDynamicSearchFlag = "IsSearchFlag";
                        else if (Flag.Contains(Flag2))
                            IsDynamicSearchFlag = "IsPageFlag";
                        else
                            IsDynamicSearchFlag = "";
                        if (columns.Contains(ColumnName))
                        {
                ...
                try
                {
                    ...
                    if (enumDBType == S3GDALDBType.ORACLE)
                    {
                        ...
                        param = new OracleParameter("@XMLAddressDetails",
                            OracleType.Clob, ObjS3g_Location_Category_Row.XMLAddressDetails.Length,
                            ParameterDirection.Input, true, 0, 0, String.Empty,
                            DataRowVersion.Default, ObjS3g_Location_Category_Row.XMLAddressDetails);
                        command.Parameters.Add(param);
                    }
                    else
                    {
                        db.AddInParameter(command, "@XMLAddressDetails", DbType.String,
                           ObjS3g_Location_Category_Row.XMLAddressDetails);
                    }

                    db.AddInParameter(command, "@CBO_Branch_Code", DbType.String, ObjS3g_Location_Category_Row.CBO_Branch_Code);
                    db.AddInParameter(command, "@Prev_CBO_Branch_Code", DbType.String, ObjS3g_Location_Category_Row.Prev_CBO_Branch_Code);

                    if (enumDBType == S3GDALDBType.ORACLE)
                    {
                        OracleParameter param;
                        param = new OracleParameter("@XMLLOBDet",
                               OracleType.Clob, ObjS3g_Location_Category_Row.XMLLOBDet.Length,
                               ParameterDirection.Input, true, 0, 0, String.Empty,
                               DataRowVersion.Default, ObjS3g_Location_Category_Row.XMLLOBDet);
                        command.Parameters.Add(param);
                    }
                    else
                    {
                        db.AddInParameter(command, "@XMLLOBDet", DbType.String, ObjS3g_Location_Category_Row.XMLLOBDet);
                    }


                    db.AddInParameter(command, "@Effective_From", DbType.DateTime, ObjS3g_Location_Category_Row.Effective_From);//Added by Suseela
                    db.AddInParameter(command, "@Effective_To", DbType.DateTime, ObjS3g_Location_Category_Row.Effective_To);//Added by Suseela
            ...
            public int FunPubInsertLocationMaster(SerializationMode SerMode, byte[] bytesLocation_Datatable, out string strExistingMapping)
            {
                ...
                try
                {
                    ...
                    db.AddInParameter(command, "@CBO_Branch_Code", DbType.String, ObjS3g_Location_Master_Row.CBO_Branch_Code);
                    db.AddInParameter(command, "@Prev_CBO_Branch_Code", DbType.String, ObjS3g_Location_Master_Row.Prev_CBO_Branch_Code);
                    db.AddInParameter(command, "@Effective_From", DbType.DateTime, ObjS3g_Location_Master_Row.Effective_From);//Added by Suseela
                    db.AddInParameter(command, "@Effective_To", DbType.DateTime, ObjS3g_Location_Master_Row.Effective_To);//Added by Suseela
            ...
            public int FunPubSaveEMICalc(SerializationMode SerMode, byte[] bytesDictionary)
            {
                ...
                try
                {
                    ...
                    foreach (KeyValuePair<string, string> ProcPair in dctProcParams)
                    {
                        ...
                        if (ProcPair.Key == "@XMLCashflow")
                        {
                            OracleParameter param;
                            param = new OracleParameter("@XMLCashflow",
                                OracleType.Clob, ProcPair.Value.Length,
                                ParameterDirection.Input, true, 0, 0, String.Empty,
                                DataRowVersion.Default, ProcPair.Value);
                            command.Parameters.Add(param);
                        }
                        else if (ProcPair.Key == "@XMLRepayment")
                        {
                            OracleParameter param;
                            param = new OracleParameter("@XMLRepayment",
                                OracleType.Clob, ProcPair.Value.Length,
                                ParameterDirection.Input, true, 0, 0, String.Empty,
                                DataRowVersion.Default, ProcPair.Value);
                            command.Parameters.Add(param);
                        }
                        else if (ProcPair.Key == "@XML_RepaymentStructure")
                        {
                            OracleParameter param;
                            param = new OracleParameter("@XML_RepaymentStructure",
                                OracleType.Clob, ProcPair.Value.Length,
                                ParameterDirection.Input, true, 0, 0, String.Empty,
                                DataRowVersion.Default, ProcPair.Value);
                            command.Parameters.Add(param);
            ...
            public int FunPubUpdateAddressNameSetup(SerializationMode SerMode, byte[] byteAddressNameSetupDetails)
            {
                try
                {
                    SystemAdmin.S3G_SYSAD_ADDSETUPDataTable obj_sysad_Address_Name_SetupDataTable;
                    DbCommand command = db.GetStoredProcCommand("S3G_SA_UPD_ADRS_NAME_SETUP");
                    obj_sysad_Address_Name_SetupDataTable = (S3GBusEntity.SystemAdmin.S3G_SYSAD_ADDSETUPDataTable)ClsPubSerialize.DeSerialize(byteAddressNameSetupDetails, SerMode, typeof(S3GBusEntity.SystemAdmin.S3G_SYSAD_ADDSETUPDataTable));
                    SystemAdmin.S3G_SYSAD_ADDSETUPRow ObjTargetRow = obj_sysad_Address_Name_SetupDataTable.NewS3G_SYSAD_ADDSETUPRow();
                    ObjTargetRow = obj_sysad_Address_Name_SetupDataTable.Rows[0] as S3GBusEntity.SystemAdmin.S3G_SYSAD_ADDSETUPRow;


                    db.AddInParameter(command, "@COMPANY_ID", DbType.Int32, ObjTargetRow.Company_ID);
                    db.AddInParameter(command, "@LOOKUP_TYPE", DbType.Int32, ObjTargetRow.Lookup_Type);
                    db.AddInParameter(command, "@EFFECTIVE_FROM", DbType.DateTime, ObjTargetRow.Effective_From);
                    db.AddInParameter(command, "@EFFECTIVE_TO", DbType.DateTime, ObjTargetRow.Effective_To);
                    db.AddInParameter(command, "@CREATED_BY", DbType.Int32, ObjTargetRow.Created_By);

                    S3GDALDBType enumDBType = Common.ClsIniFileAccess.S3G_DBType;
                    if (enumDBType == S3GDALDBType.ORACLE)
                    {
                        OracleParameter param;
                        param = new OracleParameter("@XML_ADD_NAME_DETAILS",
                            OracleType.Clob, ObjTargetRow.XML_Address_Name_Details.Length,
                            ParameterDirection.Input, true, 0, 0, String.Empty,
                            DataRowVersion.Default, ObjTargetRow.XML_Address_Name_Details);
                        command.Parameters.Add(param);
                    }
                    else
                    {
                        db.AddInParameter(command, "@XML_ADD_NAME_DETAILS", DbType.String, ObjTargetRow.XML_Address_Name_Details);
                    }
            ...
            public int FunPubInsertUpdateEntityType(SerializationMode SerMode, byte[] byteEntityTypeDetails)
            {
                try
                {
                    SystemAdmin.S3G_SYSAD_ENTITY_TYPEDataTable obj_Entity_Type_DataTable;
                    DbCommand command = db.GetStoredProcCommand("S3G_SA_INS_UPD_ENTITY_TYPE");
                    obj_Entity_Type_DataTable = (S3GBusEntity.SystemAdmin.S3G_SYSAD_ENTITY_TYPEDataTable)ClsPubSerialize.DeSerialize(byteEntityTypeDetails, SerMode, typeof(S3GBusEntity.SystemAdmin.S3G_SYSAD_ENTITY_TYPEDataTable));
                    SystemAdmin.S3G_SYSAD_ENTITY_TYPERow objTargetRow = obj_Entity_Type_DataTable.NewS3G_SYSAD_ENTITY_TYPERow();
                    objTargetRow = obj_Entity_Type_DataTable.Rows[0] as S3GBusEntity.SystemAdmin.S3G_SYSAD_ENTITY_TYPERow;

                    db.AddInParameter(command, "@Company_ID", DbType.Int32, objTargetRow.Company_ID);
                    db.AddInParameter(command, "@Entity_Type_ID", DbType.Int32, objTargetRow.Entity_Type_ID);
                    db.AddInParameter(command, "@Entity_Type_Name", DbType.String, objTargetRow.Entity_Type_Name);
                    db.AddInParameter(command, "@Is_Active", DbType.Boolean, objTargetRow.Is_Active);

                    S3GDALDBType enumDBType = Common.ClsIniFileAccess.S3G_DBType;
                    if (enumDBType == S3GDALDBType.ORACLE)
                    {
                        OracleParameter param;
                        param = new OracleParameter("@XMLPROGRAM_DET",
                            OracleType.Clob, objTargetRow.XML_Program_Details.Length,
                            ParameterDirection.Input, true, 0, 0, String.Empty,
                            DataRowVersion.Default, objTargetRow.XML_Program_Details);
                        command.Parameters.Add(param);
                    }
                    else
                    {
                        db.AddInParameter(command, "@XMLPROGRAM_DET", DbType.String, objTargetRow.XML_Program_Details);
                    }

                    if (enumDBType == S3GDALDBType.ORACLE)
                    {
                        OracleParameter param;
                        param = new OracleParameter("@XMLCONSDOCUMENTSDET",
                            OracleType.Clob, objTargetRow.XML_Const_Doc_Det.Length,
                            ParameterDirection.Input, true, 0, 0, String.Empty,
                            DataRowVersion.Default, objTargetRow.XML_Const_Doc_Det);
                        command.Parameters.Add(param);
                    }
                    else
                    {
                        db.AddInParameter(command, "@XMLCONSDOCUMENTSDET", DbType.String, objTargetRow.XML_Const_Doc_Det);
                    }

                    db.AddInParameter(command, "@User_ID", DbType.Int32, objTargetRow.Created_By);
            ...
            #region SMS SETUP - UPDATE SMS SCHEDULED STATUS

            public int FunPubUpdateSMSStatus(string strXMLSMSData)
            {
                intErrorCode = 0;

                try
                {
                    DbCommand command = db.GetStoredProcCommand("S3G_UPDATE_SMS_SCHDULED_STATUS");

                    S3GDALDBType enumDBType = Common.ClsIniFileAccess.S3G_DBType;
                    if (enumDBType == S3GDALDBType.ORACLE)
                    {
                        OracleParameter param;
                        param = new OracleParameter("@XML_SMS_DTL",
                            OracleType.Clob, strXMLSMSData.Length,
                            ParameterDirection.Input, true, 0, 0, String.Empty,
                            DataRowVersion.Default, strXMLSMSData);
                        command.Parameters.Add(param);
                    }
                    else
                    {
                        db.AddInParameter(command, "@XML_SMS_DTL", DbType.String, strXMLSMSData);
                    }
                    db.FunPubExecuteNonQuery(command);
                }
                catch (Exception ex)
